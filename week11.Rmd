...
2. The Our World in Data COVID-19 dataset is perfect for performing a PCA. Use the code below that generates the `owid_total` data table. This contains a set of variables from the OWID dataset and the total cases per million by the beginning of November 2021.
    ```{r}
    owid_total = 
      read_csv("https://covid.ourworldindata.org/data/owid-covid-data.csv") %>% 
      filter(date == "2021-11-01", !is.na(continent)) %>% 
      select(location, total_cases_per_million, stringency_index, people_vaccinated_per_hundred, population_density, median_age, male_smokers, life_expectancy) %>% 
