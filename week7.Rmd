...
- If you haven't started playing with your dataset, start doing so now.
...
To see how to use the pipe `%>%`, lets first load the New York Times COVID-19 daily case and death data for the US.
```{r, message=FALSE}
library(tidyverse)
...
us = left_join(us, pop %>% mutate(population = `2020`) %>% select(state, population)) %>%
  mutate(cases_per_100k = signif(cases/population*1e5, digits = 3), deaths_per_100k = signif(deaths/population*1e5, digits = 3)) %>%
...
```
The above code also loads in estimated population sizes for each state from [US Census data](https://www.census.gov/programs-surveys/popest/technical-documentation/research/evaluation-estimates/2020-evaluation-estimates/2010s-totals-national.html) and gets rid of rows for which there aren't population data. We also add a few columns for the day and month that will be helpful later and remove days where cases were negative (which was a way for states to correct prior mistakes in reporting).

These data are already pretty tidy. But what could we do to make them **even more tidy**? 
```{r}
xtra_tidy_us = pivot_longer(us, c(cases, cases_avg, cases_avg_per_100k, deaths, deaths_avg, deaths_avg_per_100k), names_to = "statistic", values_to = "value")
```
What if we then wanted to remove the column `geoid`? We could do this:
```{r}
xtra_tidy_us = select(xtra_tidy_us, -geoid)
```

Those tidying operations can be put together with the pipe operator `%>%`. The pipe allows you to easily chain together operations so that its easier to read them from left to right. For example, our previous two operations could be put together like
```{r}
xtra_tidy_us = us %>% 
  pivot_longer(c(cases, cases_avg, cases_avg_per_100k, deaths, deaths_avg, deaths_avg_per_100k), names_to = "statistic", values_to = "value") %>%
  select(-geoid)
```
Effectively, the pipe takes what is on the left side of it and uses it as the first argument in the function on the right side. In the above example, `us`, our initial data table, is on the left and it gets used as the first argument in `pivot_longer`. The `pivot_longer` function returns a `data.frame`, which is then used as the argument to the function of the right of the next pipe, which is `select`. The rule of thumb then is that if you want to convert a command without a pipe to one where you use a pipe, take the first argument out of the function, put it on the left of the function separated by the pipe. Likewise, to get rid of the pipe, take the input on the left and put it in the first argument of the function on the right of the pipe and delete the pipe. 

Using the pipe, we can then make a whole sequence of command look much simpler. For example, suppose we wanted a data table with just the cases in Kentucky, West Virginia, and Tennessee for the months of August and September 2021
```{r}
ky_wv_tn_cases_aug_sept_2021 = 
  us %>%
  select(-contains("deaths")) %>%
  select(-geoid) %>%
  filter(state %in% c("Kentucky", "West Virginia", "Tennessee")) %>%
  filter(date >= "2021-08-01" & date <= "2021-09-30")
```
...
**Finally, a very significant advantage** of structuring your slicing / wrangling commands this way when you use RStudio is that RStudio will help you auto-complete column names. As you've probably seen already, typing these names can get cumbersome, so this is a big help. RStudio doesn't do this when you put the `data.frame` in the function directly (for some unknown reason).

# Smoothed line plots

In previous weeks, you have produced simple scatter and line plots. With these the case data, we can produce a scatter plot with `geom_point`.
```{r}
ky_wv_tn_plot = ky_wv_tn_cases_aug_sept_2021 %>% ggplot(aes(x=date, y=cases_per_100k, color=state))
...
```
Producing a line through these points isn't that helpful in showing a trend because the data is very spikey. This has to do with states often not reporting data on the weekends and then adding weekend data to the next business day.
...
If your data has discrete categories (i.e., "categorical" data), then you may want a simple bar plot. As an example, we can create a bar plot with how many days cases per 100k exceeded 100 for Kentucky, West Virginia, and Tennessee
```{r}
...
```{r}
us %>% group_by(state) %>% summarize(total_avg_deaths_per_100k = mean(deaths_per_100k))
```

To get a histogram of the margin, use the `geom_histogram` function. We can for example look at how many days across all the states had a certain number of cases per 100k.
```{r}
us %>% ggplot() +
...
You might have noticed that R keeps reminding you to adjust the `binwidth`. This is simply the width of the bins on the x-axis. For example, a binwidth of 0.1 yields
```{r}
us %>% ggplot() +
  geom_freqpoly(aes(x = log10(1 + cases_per_100k), color=day), binwidth = 0.1)
```
...
Notice how the heights of the curves have shifted. This is because the area under the curve of the KDE must sum to one and the y-axis measures the density, or approximately the probability, of a specific log10 number of cases per 100k. The histogram can be normalized too by dividing each curve by the total number of days in the dataset.
...
```{r}
us %>% ggplot(aes(x = reorder(state, log10(1 + cases_per_100k), FUN = median), y = log10(1 + cases_per_100k))) +
  geom_boxplot(outlier.color = "red") +
  coord_flip()
```
...
4. Plot boxplots of the deaths per 100k (not `avg`!) for each day of the week using the `us` data table we created at the beginning of the lecture.
    ...
    - Bonus! Use `facet_wrap` to get boxplots for each day for each state. Looking at the boxplots, which state do you think reports deaths mostly on just ONE day of the week?
