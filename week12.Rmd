...
```{r, message=FALSE, fig.cap = "Regulatory relationships in E. coli extracted from the RegulonDB database, see Kolaczyk and Csárdi, 2020, p. 121 [^1]"}
library(sand)
data(Ecoli.data)
...
```{r}
g = graph( c("catherine", "david weis", "david weis", "vinnie", 
...
```{r}
V(g)$position = c("assoc", "full", "full", "assoc", "assist", "assist", "assoc", "assoc")
vertex_attr(g)
```
...
macaque_graph %>%
  ...
  coord_fixed()
```
A cool thing about `ggraph` though is that it has a few interesting layouts. For example, to do a circular graph, we say we want a `linear` layout with edges that are arcs using `geom_edge_arc`:
```{r}
macaque_graph %>%
